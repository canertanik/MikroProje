...
import { Filter, Calendar, FileText, ArrowRightLeft, Truck } from 'lucide-react';
import toast from 'react-hot-toast';

import { getCurrentAccounts } from '../features/current-accounts/api';
import { getSupplierStatement } from '../features/supplier-statements/api';
...
export const SupplierStatement = () => {
  ...
  // Filter for Supplier and Both
  const supplierAccounts = useMemo(() => {
    if (!accountsData) return [];
    return accountsData.filter(
      (a) => a.type === CurrentAccountType.Supplier || a.type === CurrentAccountType.Both
    );
  }, [accountsData]);

  const accountOptions = useMemo(() => {
    return supplierAccounts.map((a) => ({
      value: a.id,
  ...
  const selectedAccount = useMemo(() => {
    return supplierAccounts.find((a) => a.id === selectedAccountId) || null;
  }, [selectedAccountId, supplierAccounts]);
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['supplier-statement', selectedAccountId, page, pageSize, startDate, endDate],
    queryFn: () =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-gray-900">Tedarikçi Ekstresi</h1>
      </div>
      ...
      <div className="bg-white p-6 border border-gray-100 rounded-xl shadow-sm space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Tedarikçi Seçimi</label>
            <Select
              ...
              placeholder="Ekstresini görmek istediğiniz tedarikçiyi seçin..."
      ...
      {selectedAccount && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-center">
            <p className="text-sm font-medium text-gray-500 mb-1">Tedarikçi Adı / Ünvanı</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {!selectedAccountId ? (
          <div className="py-20 flex flex-col items-center justify-center text-center px-4">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-1">Tedarikçi Seçilmedi</h3>
            <p className="text-gray-500 max-w-sm">Ekstreyi görüntülemek için lütfen yukarıdan bir tedarikçi seçin.</p>
          </div>
        ) : isError ? (
          <div className="py-20 flex flex-col items-center justify-center text-center px-4">
